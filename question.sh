...
#
sed "s/\//-/g" data.csv | sed "s/-\([0-9][0-9]\);/-20\1;/" | sed "s/-\([0-9]\)-/-0\1-/" | sed "s/^\([0-9]\)-/0\1-/" | sed "s/^\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9][0-9]*\);/\3-\2-\1;/" | sed "s/;\([a-z]\)/;\U\1/g" | sed "s/;N/;\\\N/g" | sed "s/;;/;\\\N;/g" | sed "s/\;$/;\\\N/g" | sed "s/[;]/,/g" | sed "s/\([0-9][0-9][0-9]\),/\1./g" 